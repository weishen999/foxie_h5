<template>
  <div class="p-list-wrapper">
    <h2>{{title}} <span class="more" @click="$router.push(router)">更多></span></h2>
    <slot>
      <!-- 图类型 -->
      <ul class="p-list img" v-if="type === 'img'">
        <li class="img__item" v-for="item in list">
      ...
      <div class="p-list alone" v-if="type === 'alone'">
        <img class="img" :src="list[0].pic" alt="">
        <p class="desc"></p>
      </div>
      <!-- 左右滑动 -->
      ...
      <ul class="p-list horizontal-2" v-if="type === 'horizontal-2'">
        <li class="img__item" v-for="item in list">
          <img class="img" :src="item.pic" alt="">
          <div class="info">
            <h3>{{item.title}}adsfadf</h3>
            <p>{{ item.remark }}asdfadf</p>
          </div>
        </li>
      </ul>
      <!-- 图文混合 -->
